...
export default function Riwayat() {
  ...
  const fetchRiwayat = () => {
    ...
    fetch('https://carefree-balance-production-20cb.up.railway.app/api/riwayat', {
      headers: { 
  ...
  const handleHapus = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://carefree-balance-production-20cb.up.railway.app/api/riwayat/${id}`, {
        method: 'DELETE',
        headers: { 
          Authorization: `Bearer ${token}` 
        },
